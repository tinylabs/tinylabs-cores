...
private:
	...
	struct Block
	{
		...
		char cachelineFiller0[MC_CACHE_LINE_SIZE - sizeof(weak_atomic<size_t>) - sizeof(size_t)];
		weak_atomic<size_t> tail;	// (Atomic) Elements are enqueued here
		size_t localFront;
		
		char cachelineFiller1[MC_CACHE_LINE_SIZE - sizeof(weak_atomic<size_t>) - sizeof(size_t)];	// next isn't very contended, but we don't want it on the same cache line as tail (which is)
...
private:
	...
	char cachelineFiller[MC_CACHE_LINE_SIZE - sizeof(weak_atomic<Block*>)];
...
{
private:
	typedef ::mc::ReaderWriterQueue<T, MAX_BLOCK_SIZE> ReaderWriterQueue;
